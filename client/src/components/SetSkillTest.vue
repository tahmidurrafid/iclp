<template>
    <div>
        <div class = "setquiz" v-if="activePart=='setQuiz'">
            <div class = "title">Set New Skill Test</div>
            <div class = "content">
                ...
                <div class = "set-time">
                    <a class = "has-time selected">Number of correct answer needed to qualify as Intermediate</a>
                    <select v-model="passmarkinter" style="text-align-last:center;font-weight:600;">
                            <option disabled value="0">Select</option>
                            <option v-for="i in 30" v-bind:value="i" v-bind:key="i">{{i}}</option>
                    </select>
                </div>
                <div class = "set-time">
                    <a class = "has-time selected">Number of correct answer needed to qualify as Advance</a>
                    <select v-model="passmarkadvance" style="text-align-last:center;font-weight:600;">
                            <option disabled value="0">Select</option>
                            <option v-for="i in 30" v-bind:value="i" v-bind:key="i">{{i}}</option>
                    </select>
                </div>
            ...
            <div class="error" v-if="errorType=='passmark'">
                <div>
                    * Required mark can't be greater than total marks.
                </div>
            </div>
            ...
            <div class="error" v-if="errorType=='levelError'">
                <div>
                    * Intermediate should be less skilled than Advanced
                </div>
            </div>
        </div>
...
export default {
    ...
    methods:{
        submitQuiz:function(){
            ...
            if(this.passmarkinter>this.displayQus||this.passmarkadvance>this.displayQus)
            {
                this.errorType="passmark"
            }
            if(this.passmarkinter>this.passmarkadvance)
            {
                this.errorType="levelError"
            }
            ...
            if(this.errorType=="")
            {
                let quiz={
                    ...
                    passmarkinter:this.passmarkinter,
                    passmarkadvance:this.passmarkadvance,

                };
                let formData = new FormData();
                formData.append('categoryID',this.$route.query.id);
                formData.append('quiz',JSON.stringify(quiz));
                axios.put( 'api/quiz/setCategoryQuiz',
                    formData,
                    {
                        headers: {
